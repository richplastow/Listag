...
Listag = (function() {
  ...
  Listag.prototype.add = function(node, id, tags) {
    ...
    for (k = 0, len1 = tags.length; k < len1; k++) {
      tag = tags[k];
      node.listagL[tag] = this.length[tag] ? this.tail[tag] : null;
      node.listagR[tag] = null;
      if (this.length[tag]) {
        this.tail[tag].listagR[tag] = node;
      } else {
        this.head[tag] = node;
        this.length[tag] = 0;
      }
...
tudor.add([
  ...
  }, "'all' `length`, `head`, `tail`, `listagL` and `listagR` as expected", tudor.equal, "13 nodes created during the '02 Listag::add()' test", 13, function(listag) {
