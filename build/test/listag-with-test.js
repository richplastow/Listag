...
(function(_oG) {
/*! Listag 0.0.6 //// MIT Licence //// http://listag.richplastow.com/ */
var Listag, Node, Tudor, _o, _oT, _oV, tudor,
...
Listag = (function() {
  ...
  Listag.prototype.add = function(cargo, id, tags) {
    var M, i, j, k, len, len1, node, tag, tmp, v;
    if (tags == null) {
      tags = [];
    }
    ...
    for (k = 0, len1 = tags.length; k < len1; k++) {
      tag = tags[k];
      node.previous[tag] = this.length[tag] ? this.tail[tag] : null;
      node.next[tag] = null;
      if (this.length[tag]) {
...
tudor.add([
  "02 Listag::add()", tudor.is, "`add()` is a function which returns a string", function() {
    ...
    return listag.tail.all.cargo.x;
  }, "A Listag instance can be recorded in itself", 'the_second', function(listag) {
    listag.add(listag);
    return listag.tail.all.previous.all.cargo.x;
  ...
  }, "'all' `length`, `head`, `tail`, `previous` and `next` as expected", tudor.equal, "13 nodes created during the '02 Listag::add()' test", 13, function(listag) {
    return listag.length.all;
  }, "Traversing rightward from `head` takes 13 steps", 13, function(listag) {
    var i, node;
    i = 0;
    ...
    return i;
  }, "Traversing leftward from `tail` takes 13 steps", 13, function(listag) {
    var i, node;
    i = 0;
    ...
    return listag.head.all.cargo.x;
  }, "The rightmost node is 'the_last'", 'the_last', function(listag) {
    return listag.tail.all.cargo.x;
  }, "The leftmost node’s leftward node is null", null, function(listag) {
    return listag.head.all.previous.all;
  }, "The rightmost node’s rightward node is null", null, function(listag) {
    return listag.tail.all.next.all;
  }, "The leftmost node’s `next.all` is 'the_second'", 'the_second', function(listag) {
    return listag.head.all.next.all.cargo.x;
  }, "The rightmost node’s `previous.all` is 'second_from_last'", 'second_from_last', function(listag) {
    return listag.tail.all.previous.all.cargo.x;
  }, "'dog' `length`, `head`, `tail`, `previous` and `next` as expected", "2 'dog' nodes created during the '02 Listag::add()' test", 2, function(listag) {
...
tudor.add([
  "03 Listag::read()", tudor.is, "`read()` is a function which returns an object", function() {
    return [new Listag];
  }, "`read()` is a function", _o.F, function(listag) {
    return listag.read;
  }, "`read('the_first')` returns an object", _o.O, function(listag) {
    listag.add({
      x: 'ok'
    }, 'the_first');
